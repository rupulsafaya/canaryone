...
import { symbolSvgNode, faviconLinkTag } from './brand.js';
...
export function renderNextReport(data: RunData): string {
  ...
  const title = `Canary One · ${escapeHtml(pdata.meta.repoName)} · ${escapeHtml(pdata.meta.dateIso)}`;
    ...
    <svg viewBox="0 0 900 580" width="100%" height="auto" role="img" aria-label="cost vs quality scatter" id="chart-svg" xmlns="http://www.w3.org/2000/svg">
      ...
      <g class="svg-header">
        ...
        <!-- Byline, right-aligned to the plot's right edge (dotted-line end at x=860).
             The brand mark is the symbol vector rather than the name set in type,
             but the domain stays as text: this band is inside the SVG so that PNG
             exports carry it, and an exported chart travels without context, so a
             mark alone would attribute the chart to nothing a stranger can act on.
             The mark exports at 2x (see the PNG export handler), so 16px lands on
             32 real pixels. "canaryone.ai" is 12 characters of 10.5px monospace,
             advancing ~0.6em, so it ends flush with the strapline below at x=860. -->
        <a href="https://canaryone.ai" target="_blank" rel="noopener" style="cursor:pointer">
          <text x="757" y="34" text-anchor="end" font-family="ui-monospace, Menlo, Monaco, monospace" font-size="10.5" fill="#6b7280">Generated by</text>
          ${symbolSvgNode(760, 21.5, 16)}
          <text x="783" y="34" text-anchor="start" font-family="ui-monospace, Menlo, Monaco, monospace" font-size="10.5" fill="#0f172a" font-weight="600" text-decoration="underline">canaryone.ai</text>
        </a>
